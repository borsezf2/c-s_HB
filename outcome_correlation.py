...
def general_outcome_correlation(adj, y, alpha, num_propagations, post_step, alpha_term, device='cpu', display=True):#cuda
...
def label_propagation(data, split_idx, A, alpha, num_propagations, idxs):
    print("label propagatopn_____________________________________________________")
    labels = data.y.data
    print("labels = ",labels)
    ...
    print("label_idx len= ",len(label_idx))
    y[label_idx] = F.one_hot(labels[label_idx],c).float().squeeze(1) 


    return general_outcome_correlation(A, y, alpha, num_propagations, post_step=lambda x:torch.clamp(x,0,1), alpha_term=True)

def double_correlation_autoscale(data, model_out, split_idx, A1, alpha1, num_propagations1, A2, alpha2, num_propagations2, scale=1.0, train_only=False, device='cpu', display=True):#cuda
...
def double_correlation_fixed(data, model_out, split_idx, A1, alpha1, num_propagations1, A2, alpha2, num_propagations2, scale=1.0, train_only=False, device='cpu', display=True):#cuda
    train_idx, valid_idx, test_idx = split_idx
    if train_only:
        label_idx = torch.cat([split_idx['train']])
        residual_idx = split_idx['train']
...
def only_outcome_correlation(data, model_out, split_idx, A, alpha, num_propagations, labels, device='cpu', display=True):#cuda
