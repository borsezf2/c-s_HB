...
def main():
    ...
    if args.dataset=="cora":
        dataset = Planetoid(root="./dataset/cora/" ,name='Cora',transform=T.ToSparseTensor())
        pre_dataset = dataset[0]
        ...
        data = dataset[0]
        # print("arvix adj t = ",data.adj_t.shape)
        print("arxiv adj t = ",data)
        print("arxiv is undirected? = ",data.is_undirected())
    # while True:
    #     pass
    ...
    if args.dataset=="cora":
        cora_train = torch.tensor([x for x in range(0,140)])
        cora_val = torch.tensor([x for x in range(140,640)])
        cora_test = torch.tensor([x for x in range(1708,2708)])
    ...
    if args.dataset=="cora":
        preprocess_data = Planetoid(root="./dataset/cora/" ,name='Cora')[0]
    else:
