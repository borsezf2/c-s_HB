...
def spectral(data, post_fix):
    ...
    Main.include("/media/harsh/forUbuntu/Common_Drive/IIT_KGP/Subgroup_generalization/c and s/CorrectAndSmooth/norm_spec.jl")
...
def preprocess(data, preprocess = "diffusion", num_propagations = 10, p = None, alpha = None, use_cache = True, post_fix = ""):
    ...
    print("diffusion data = ",data)
    print('Computing adj...')
    # if data.x.shape[0]==2708:
    #     N = data.x.shape[0]
    # else:

    N = data.num_nodes
    print("data.edge_index1 = ",data.edge_index)
    data.edge_index = to_undirected(data.edge_index, data.num_nodes)
    print("data.edge_index2 = ",data.edge_index)
    row, col = data.edge_index
    adj = SparseTensor(row=row, col=col, sparse_sizes=(N, N))
    adj = adj.set_diag()
